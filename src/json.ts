...
import { Class, constructorOf, JsonObject } from "./util";
...
export function DeferAll<T extends string[], R>(referenceKeys: T, defer: (...args: T) => R): Function {
    return function(target: any, propertyKey: string) {
        const element = decoratedOf(target);
...
const classChecks: Class<any>[] = [Date];
function getTypeString(value: any, classes: Class<any>[] = [Date]) {
    for (let clat of classes.concat(classChecks))
        if (value instanceof clat) return clat.name;
    return typeof(value);
}

export function generateClass(raw: JsonObject<any>, options?: GenerateClassOptions): string | void {
